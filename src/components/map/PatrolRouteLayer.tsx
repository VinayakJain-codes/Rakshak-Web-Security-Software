  ...
  useEffect(() => {
    ...
    try {
      if (expectedCoords.length > 1) {
        const expectedPolyline = mapplsObject.Polyline({
          map: map,
      ...
      if (actualCoords.length > 1) {
        const actualPolyline = mapplsObject.Polyline({
          map: map,
          paths: actualCoords,
          strokeColor: '#4c5e8b',
          strokeOpacity: 1,
      ...
      console.error('Error creating patrol route layers:', e);
    }
    ...
    return () => {
      layersRef.current.forEach(layer => {
        if (layer) {
          try {
            mapplsObject.remove({ map: map, layer: layer });
          } catch(e) {}
        }
      });
      layersRef.current = [];
    };
  }, [map, mapplsObject, route]);
